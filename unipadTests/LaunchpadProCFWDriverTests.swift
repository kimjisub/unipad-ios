...
struct LaunchpadProCFWDriverTests {
    ...
    @Test func padNotesMapToRowsAndColumns() {
        ...
        driver.getSignal(cmd: 9, sig: -112, note: 36, velocity: 100)  // first note: bottom-left
        driver.getSignal(cmd: 9, sig: -112, note: 99, velocity: 100)  // last note: top-right
        driver.getSignal(cmd: 8, sig: -128, note: 36, velocity: 0)    // note-off: release
    ...
    @Test func padLedRoundTripsThroughTheSameNote() {
        ...
        for x in 0..<8 {
            for y in 0..<8 {
                ...
                #expect(capture.sent[0].cmd == 9 && capture.sent[0].sig == 0x9F)  // note-on, channel 16
                capture.pads.removeAll()
                driver.getSignal(cmd: 9, sig: -112, note: Int(capture.sent[0].note), velocity: 1)
    ...
    @Test func ringButtonsMapToChainsAndFunctionKeys() {
        let (driver, capture) = make()
        driver.getSignal(cmd: 9, sig: -112, note: 28, velocity: 1)   // top row, leftmost
        driver.getSignal(cmd: 9, sig: -112, note: 100, velocity: 1)  // right column, top
        driver.getSignal(cmd: 9, sig: -112, note: 123, velocity: 1)  // bottom row, rightmost
        driver.getSignal(cmd: 9, sig: -112, note: 115, velocity: 1)  // left column, bottom
        driver.getSignal(cmd: 9, sig: -112, note: 27, velocity: 1)   // top-right corner
        #expect(capture.keys.map(\.f) == [0, 8, 16, 24, 32])
        #expect(capture.chains.map(\.c) == [0, 8, 16])
    }

    @Test func controlChangeAndUnknownNotes() {
        let (driver, capture) = make()
        driver.getSignal(cmd: 11, sig: -80, note: 36, velocity: 127)  // CC with value: press
        driver.getSignal(cmd: 11, sig: -80, note: 36, velocity: 0)    // CC value 0: release
        driver.getSignal(cmd: 9, sig: -112, note: 20, velocity: 1)    // not on the map
        driver.getSignal(cmd: 10, sig: -96, note: 36, velocity: 1)    // aftertouch: ignored
        #expect(capture.pads.map(\.down) == [true, false])
    ...
    @Test func initSendsPerformanceModeAndClearOnce() {
        let (driver, capture) = make()
        driver.initialize()
